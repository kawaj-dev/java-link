...
sequenceDiagram
    ...
    Note over Course: LessonProgressServiceを介してHttpSessionから進捗を取得
    ...
    alt 不正解または回答できないStep
        ...
        Note over Engine: QuizServiceで正誤を判定し、LessonProgressを更新
        Note over Engine: LessonProgressServiceを介してHttpSessionへ進捗を保存
